{ config, pkgs, lib, ... }:
{
  ...
  home.activation.installFlatpaks = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak remote-add --if-not-exists --system flathub https://dl.flathub.org/repo/flathub.flatpakrepo

    # Install applications
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub com.adobe.Reader
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub com.bitwarden.desktop
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub com.discordapp.Discord
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub com.obsproject.Studio
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub com.spotify.Client
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub org.flameshot.Flameshot
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub org.gimp.GIMP
    $DRY_RUN_CMD ${pkgs.flatpak}/bin/flatpak install --system -y flathub org.videolan.VLC
  '';
}
